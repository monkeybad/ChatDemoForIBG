...
- (OTRXMPPManager*) getXMPPManager
{
    NSArray* allLoginAccounts = [OTRAccountsManager allAccountsAbleToAddBuddies];
    for (OTRManagedAccount* account in allLoginAccounts)
    {
        NSObject <OTRProtocol> * protocol = [protocolManagers objectForKey:account.uniqueIdentifier];
        if(protocol)
        {
            ...
            {
                return (OTRXMPPManager*)protocol;
            }
        }
    }
    return nil;
