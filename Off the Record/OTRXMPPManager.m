...
@interface OTRXMPPManager() <XMPPRoomStorage,XMPPRoomDelegate>
...
@property (nonatomic, strong) id roomGetListBlk;
@property (nonatomic, strong) id roomCreatedRoomBlk;
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
    ...
    if ([[iq elementID] isEqualToString:[self.JID user]])
    {
        NSString* from = [iq fromStr];
        if ([from rangeOfString:@"conference."].length > 0)
        {
            if (self.roomGetListBlk != nil)
            {
                BOOL succeed = NO;
                NSMutableDictionary* itemsDict = nil;
                NSArray* elements = [iq children];
                for (NSXMLElement* element in elements)
                {
                    if ([[element name] isEqualToString:@"query"])
                    {
                        ...
                        for (NSXMLElement* elm in items)
                        {
                            NSString* jid = [elm attributeStringValueForName:@"jid"];
                            NSString* name = [elm attributeStringValueForName:@"name"];
                            [itemsDict setObject:name forKey:jid];
                        }
                        break;
                    }
                    else if ([[element name] isEqualToString:@"error"])
                    {
                        succeed = NO;
                        break;
                    }
                }
                void (^resultBlk)(BOOL, NSDictionary*) = self.roomGetListBlk;
                if (!succeed)
                {
                    itemsDict = nil;
                }
                resultBlk(succeed,itemsDict);
                self.roomGetListBlk = nil;
            }
        }
    }
...
#pragma mark - <Rooms API>
- (void)requestGroupList:(void (^)(BOOL isSucceed, NSDictionary* roomJIDKeyAndNameValueDict))resultBlk
{
    if (resultBlk)
    {
        self.roomGetListBlk = resultBlk;
    }
    NSString* roomListJIDStr = [NSString stringWithFormat:@"conference.%@",[self.JID domain]];
    NSXMLElement* doc = [NSXMLElement elementWithName:@"query" xmlns:@"http://jabber.org/protocol/disco#items"];
    XMPPIQ* iq = [XMPPIQ iqWithType:@"get" elementID:[self.JID user] child:doc];
    [iq addAttributeWithName:@"to" stringValue:roomListJIDStr];
...
- (void)enterRoomWithOnReceivedRoomMsgFor:(NSString*)roomJID callBack:(void (^)(NSString* roomMsg, NSString* memeberJID, NSString* memberNick))resultBlk
{
    ...
    {
        [self.roomReceiveMsgBlkDict setObject:resultBlk forKey:roomJID];
    }
...
- (void)leaveRoomFor:(NSString*)roomJID
{
    ...
    if (room)
    {
        self.roomHasLeftDict[roomJID] = @YES;
        [room leaveRoom];
        [room deactivate];
    }
...
- (void)sendRoomMsg:(NSString*)roomMsg withRoomJID:(NSString*)roomJID
{
    ...
    if (room)
    {
        NSString* uniqueID = [OTRUtilities uniqueString];
        XMPPMessage * xmppMessage = [XMPPMessage message];
        [xmppMessage addAttributeWithName:@"id" stringValue:uniqueID];
        [xmppMessage addBody:roomMsg];
        
        
        /*
        NSXMLElement* testImg = [DDXMLElement elementWithName:@"img"];
        [testImg addAttributeWithName:@"src" stringValue:@"http://www.webrtc-experiment.com/images/fork-left.png"];
        NSXMLElement* body = [DDXMLElement elementWithName:@"body"];
        [body setStringValue:roomMsg];
        [body setChildren:@[testImg]];
        [xmppMessage addChild:body];
        //[xmppMessage addChild:testImg];
         */
...
- (void)sendRoomMsg:(NSString*)roomMsg withHtml:(NSString*)htmlString withRoomJID:(NSString*)roomJID
{
    ...
    if (room)
    {
        ...
        NSXMLElement* htmlElement = [[DDXMLElement alloc] initWithXMLString:htmlString error:nil];
        if (htmlElement)
        {
            [xmppMessage addChild:htmlElement];
        }
...
- (void)xmppRoomDidLeave:(XMPPRoom *)sender
{
    ...
    if (roomJID)
    {
        left = self.roomHasLeftDict[roomJID];
    }
    if (left && ![left boolValue])
    {
        [sender joinRoomUsingNickname:[self.JID user] history:nil];
    }
}
