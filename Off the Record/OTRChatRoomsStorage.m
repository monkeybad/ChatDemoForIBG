...
@implementation OTRChatRoomsStorage
...
- (void)handleChatRoomItem:(NSXMLElement *)item xmppStream:(XMPPStream *)stream;
{
    ...
    OTRManagedGroup * user = [self groupWithJID:jid xmppStream:stream inContext:localContext];
...
-(void)updateUser:(OTRManagedGroup *)user updateWithItem: (NSXMLElement *)item
{
//    NSManagedObjectContext * context = [NSManagedObjectContext MR_contextForCurrentThread];
...
-(OTRManagedGroup *)groupWithJID:(XMPPJID *)jid xmppStream:(XMPPStream *)stream inContext:(NSManagedObjectContext *)context
{
    return [OTRManagedGroup fetchOrCreateWithName:[jid bare] inContext:context];
}
